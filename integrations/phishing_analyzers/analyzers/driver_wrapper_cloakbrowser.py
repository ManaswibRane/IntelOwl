...
logger = setup_file_logger("driver_wrapper_cloakbrowser")
...
def _get_post_body(request: Request) -> bytes | None:
    """Safely extract the raw POST body bytes from a Cloakbrowser request, or None on failure."""
    try:
...
class CloakbrowserDriverWrapper:
    DEFAULT_USER_AGENT = (
    ...
    def __init__(
        ...
        """
        Initialise the Cloakbrowser driver wrapper.
    ...
    def _close_browser(self):
        ...
        try:
            ...
            logger.warning(f"Error while closing browser: {e} for url {self.last_url}")

    def _init_driver(self):
        """Launch the Chromium browser and create the initial context and page."""
        logger.info(f"Initialising Cloakbrowser (proxy={self.proxy!r})")

        self._browser = launch()
        self._create_context_and_page()
        logger.info(f"Cloakbrowser driver initialised successfully for {self.last_url}")
    ...
    def close(self):
        ...
        logger.info(f"Closing Cloakbrowser page for {self.last_url} ")
    ...
    def quit(self):
        """Fully shut down the browser, Cloakbrowser context, and clean up the temporary HAR file."""
        logger.info("Quitting Cloakbrowser")
        try:
            self._close_browser()
