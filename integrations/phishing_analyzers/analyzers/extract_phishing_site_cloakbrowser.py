...
from driver_wrapper_cloakbrowser import CloakbrowserDriverWrapper
from logging_setup import setup_file_logger
from playwright_request_serializer import dump_playwright_request

logger = setup_file_logger("extract_phishing_site_cloakbrowser")


def extract_driver_result(driver_wrapper: CloakbrowserDriverWrapper) -> dict:
    logger.info("Extracting Cloakbrowser driver result...")
    driver_result: dict = {
    ...
    logger.info("Finished extracting Cloakbrowser driver result")
...
def analyze_target(
    ...
    user_agent: str = CloakbrowserDriverWrapper.DEFAULT_USER_AGENT,
):
    driver_wrapper = None
    try:
        driver_wrapper = CloakbrowserDriverWrapper(
        ...
        logger.debug(f"JSON dump of Cloakbrowser driver {result=}")

        print(result)
    except Exception as e:
        logger.exception(f"Exception during Cloakbrowser analysis of target website {target_url}: {e}")
    finally:
        if driver_wrapper:
            driver_wrapper.quit()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=CloakbrowserDriverWrapper.DEFAULT_USER_AGENT,
    )
